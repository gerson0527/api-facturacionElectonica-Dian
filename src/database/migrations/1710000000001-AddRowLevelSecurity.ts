...
const TABLES_WITH_TENANT = [
  'tenants', 'users', 'dian_software_credentials', 'digital_certificates',
  'numbering_ranges', 'customers', 'invoices', 'invoice_lines', 'tax_totals',
  'dian_submissions', 'credit_notes', 'debit_notes', 'audit_events',
];
...
export class AddRowLevelSecurity1710000000001 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    for (const table of TABLES_WITH_TENANT) {
      ...
      await queryRunner.query(`DROP POLICY IF EXISTS "${table}_tenant_isolation_update" ON "${table}"`);
      await queryRunner.query(`DROP POLICY IF EXISTS "${table}_tenant_isolation_delete" ON "${table}"`);
