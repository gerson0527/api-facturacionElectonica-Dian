    ...
    ThrottlerModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        throttlers: [
          ...
          { name: "refresh", ttl: 60000, limit: 10 },
          { name: "certificados", ttl: 60000, limit: 10 },
          { name: "facturas", ttl: 60000, limit: 100 },
          { name: "retry", ttl: 60000, limit: 20 },
          { name: "descargas", ttl: 60000, limit: 30 },
          { name: "auditoria", ttl: 60000, limit: 50 },
        ],
      }),
    }),
    BullModule.forRootAsync({
      ...
      useFactory: (config: ConfigService) => ({
        connection: {
          ...
          connectTimeout:
            config.get<string>("NODE_ENV") === "development" ? 3000 : 10000,
          retryStrategy: (times: number) => {
            if (config.get<string>("NODE_ENV") === "development" && times > 1)
              return null;
