...
describe("CertificatesService", () => {
  ...
  const configService = {
    get: jest.fn((key: string) =>
      key === "STORAGE_PATH" ? "./storage-test" : undefined,
    ),
  };
  ...
  it("upload guarda archivo cifrado y persiste certificado", async () => {
    ...
    cryptoService.encryptWithIntegrity.mockReturnValueOnce({
      ciphertext: "pfx-enc",
      iv: "iv1",
      authTag: "tag1",
      keyVersion: 1,
    ...
    certRepo.create.mockImplementation(
      (payload: Partial<DigitalCertificate>) => payload,
    );
    certRepo.save.mockImplementation(
      async (payload: Partial<DigitalCertificate>) => payload,
    );
    ...
    expect(cryptoService.encryptWithIntegrity).toHaveBeenCalledWith(
      Buffer.from("contenido-pfx").toString("base64"),
    ...
    expect(certRepo.create).toHaveBeenCalledWith({
      ...
      encryptedPasswordRef: JSON.stringify({
        ciphertext: "pass-enc",
        iv: "iv2",
        authTag: "tag2",
      }),
      encryptedPinRef: JSON.stringify({
        ciphertext: "pin-enc",
        iv: "iv3",
        authTag: "tag3",
      }),
