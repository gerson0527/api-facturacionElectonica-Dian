...
export class DebitNotesService {
  ...
  async create(
    ...
    // FIX: factura debe estar ACCEPTED para emitir ND
    if (invoice.status !== "accepted") {
      throw new BadRequestException(
        `Cannot issue debit note: invoice status is ${invoice.status}, must be 'accepted'`,
      );
    }

    // FIX: validar reasonCode del catálogo DIAN (01-05 para ND)
    const validReasonCodes = ["01","02","03","04","05"];
    if (!validReasonCodes.includes(input.reasonCode)) {
      throw new BadRequestException(
        `Invalid debit note reason code: ${input.reasonCode}`,
      );
    }
    ...
    if (Number(input.totalAmount) > Number(invoice.totalAmount)) {
      throw new BadRequestException(
        `Debit note amount ${input.totalAmount} exceeds invoice total ${invoice.totalAmount}`,
      );
    }
    ...
    return this.idempotencyService.executeWithIdempotency(
      ...
      async (payloadHash, manager) => {
        ...
        if (!softwareCreds)
          throw new NotFoundException("Credenciales DIAN no encontradas");
        const softwarePin =
        ...
        const saved = await manager.save(debitNote);
        ...
        const xmlContent =
          await this.xmlBuilderService.buildDebitNoteXml(debitNoteData);

        const xmlDir = path.join(this.storagePath, "xml", tenantId);
        await fs.mkdir(xmlDir, { recursive: true });
        const xmlPath = path.join(
          xmlDir,
          `nd_${number.replace(/[^a-zA-Z0-9]/g, "_")}.xml`,
        );
        ...
        if (cert) {
          ...
          signedXmlPath = path.join(
            xmlDir,
            `signed_nd_${number.replace(/[^a-zA-Z0-9]/g, "_")}.xml`,
          );
          ...
          zipPath = path.join(
            zipDir,
            `nd_${number.replace(/[^a-zA-Z0-9]/g, "_")}.zip`,
          );
          ...
          this.logger.log(
            `Nota débito ${number} encolada para envío DIAN (submission: ${submission.id})`,
          );
        }

        await manager.update(DebitNote, saved.id, {
          signedXmlPath: signedXmlPath || "",
          xmlPath,
        });

        const finalNote = await manager.findOne(DebitNote, {
          where: { id: saved.id },
        });

        await manager.update(DebitNote, saved.id, {
          responseSnapshot: finalNote as any,
        });

        return { snapshot: finalNote, statusCode: 201 };
      },
