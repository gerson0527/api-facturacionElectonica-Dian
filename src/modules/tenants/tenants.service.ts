...
export class TenantsService {
  ...
  async create(data: { name: string; nit: string; dv?: string; address?: string; phone?: string; email?: string; environment?: string }): Promise<Tenant> {
    const existing = await this.tenantRepo.findOne({ where: { nit: data.nit } });
    if (existing) {
