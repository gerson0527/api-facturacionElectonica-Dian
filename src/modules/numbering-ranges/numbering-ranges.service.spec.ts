...
describe("NumberingRangesService", () => {
  ...
  it("create inicializa currentNumber como fromNumber - 1", async () => {
    rangeRepo.create.mockImplementation(
      (payload: Partial<NumberingRange>) => payload,
    );
    rangeRepo.save.mockImplementation(
      async (payload: Partial<NumberingRange>) => payload,
    );
  ...
  it("findByTenant consulta solo rangos activos del tenant", async () => {
    ...
    expect(rangeRepo.find).toHaveBeenCalledWith({
  ...
  it("reserveNextNumber reserva consecutivo y actualiza currentNumber", async () => {
    ...
    expect(queryBuilderSelect.setLock).toHaveBeenCalledWith(
      "pessimistic_write",
    );
