    ...
    try {
      ...
      if (statusCode === '00' || statusCode === '0') {
        ...
        if (statusResponse.XmlBytes) {
          ...
          await this.invoiceRepo.update(invoiceId, { dianResponsePath: responsePath });
        }
        ...
        this.logger.warn(`Invoice ${invoiceId} rejected: ${statusCode} - ${statusResponse.StatusDescription}`);
        await this.submissionRepo.update(submissionId, {
          status: 'rejected',
