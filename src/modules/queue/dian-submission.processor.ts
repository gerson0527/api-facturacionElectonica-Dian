...
export class DianSubmissionProcessor extends WorkerHost {
  ...
  async process(job: Job<{ submissionId: string; invoiceId: string; tenantId: string; zipPath: string }>): Promise<any> {
    ...
    try {
      await this.submissionRepo.update(submissionId, { status: 'submitting', submittedAt: new Date() });
      ...
      // Send to DIAN
      const response = await this.dianSoapClient.sendBillAsync(fileName, contentFileBase64);
      ...
      if (response.TrackId) {
        ...
        await this.dianStatusQueue.add('dian-status', {
          trackId: response.TrackId,
          submissionId,
          invoiceId,
          tenantId: job.data.tenantId,
        }, {
          attempts: this.configService.get<number>('QUEUE_STATUS_MAX_ATTEMPTS') || 5,
