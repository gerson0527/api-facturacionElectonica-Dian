...
export class CreditNotesService {
  ...
  async create(invoiceId: string, input: { issueDate: string; reasonCode: string; totalAmount: number; prefix: string; description?: string }): Promise<CreditNote> {
    ...
    const doc = create({ version: '1.0', encoding: 'UTF-8' })
      ...
      .ele('cbc:Note').txt(input.description || `Nota crédito de la factura ${invoice.number}`).up()
      ...
      .ele('cbc:Description').txt(input.description || 'Ajuste').up()
      .up()
      ...
      .ele('cbc:ID').txt('1').up()
      .ele('cbc:CreditedQuantity', { unitCode: '94', unitCodeListID: 'UN/ECE 20' }).txt('1').up()
      .ele('cbc:LineExtensionAmount', { currencyID: 'COP' }).txt(input.totalAmount.toFixed(2)).up()
      .ele('cac:Item')
      .ele('cbc:Description').txt(input.description || 'Nota crédito').up()
      .up()
      .ele('cac:Price')
