...
function generateSelfSignedP12(password: string): Buffer {
  ...
  cert.setSubject([{ name: 'commonName', value: 'Test DIAN' }, { name: 'organizationName', value: 'Test SAS' }]);
  cert.setIssuer([{ name: 'commonName', value: 'Test CA' }, { name: 'organizationName', value: 'Test SAS' }]);
...
describe('SigningService', () => {
  ...
  it('debe firmar XML y retornar signedXml + certificateBase64', async () => {
    const xml = '<?xml version="1.0" encoding="UTF-8"?><Invoice><ID>FAC-001</ID></Invoice>';
    const result = await service.signXmlFromBuffer(xml, p12Buffer, TEST_PASSWORD);
  ...
  it('debe rechazar certificado expirado', async () => {
    ...
    const expiredP12 = forge.pkcs12.toPkcs12Asn1(keys.privateKey, cert, 'pass', { algorithm: '3des' });
    const expiredBuffer = Buffer.from(forge.asn1.toDer(expiredP12).getBytes(), 'binary');

    await expect(service.signXmlFromBuffer('<a></a>', expiredBuffer, 'pass'))
  ...
  it('debe preservar el contenido original del XML dentro del firmado', async () => {
    const xml = '<?xml version="1.0"?><Invoice><ID>FAC-001</ID><Total>1000</Total></Invoice>';
    const result = await service.signXmlFromBuffer(xml, p12Buffer, TEST_PASSWORD);
  ...
  it('debe insertar la firma dentro de ext:UBLExtensions cuando existe', async () => {
    const xml = '<?xml version="1.0"?><Invoice><ext:UBLExtensions><ext:UBLExtension><ext:ExtensionContent></ext:ExtensionContent></ext:UBLExtension></ext:UBLExtensions></Invoice>';
    const result = await service.signXmlFromBuffer(xml, p12Buffer, TEST_PASSWORD);
    expect(result.signedXml.indexOf('ds:Signature')).toBeLessThan(result.signedXml.indexOf('</ext:UBLExtensions>'));
  });

  it('debe fallar con contraseña incorrecta', async () => {
    await expect(service.signXmlFromBuffer('<a></a>', p12Buffer, 'wrong-password'))
      .rejects.toThrow();
  });
});
