...
export class DianOutboxService {
  ...
  private async executeWithRetry(
    ...
    try {
      ...
      await this.storage.save(
        submission.tenantId,
        "dian-response",
        `request-${submission.id}.zip`,
        zipContent,
      );

      locked.requestZipPath = this.storage.getFullPath(
        submission.tenantId,
        "dian-response",
        `request-${submission.id}.zip`,
      );
      ...
      if (attemptNumber < this.maxRetries) {
        ...
        this.logger.warn(
          `Intento ${attemptNumber}/${this.maxRetries} falló. Reintentando en ${delay}ms...`,
        );
  ...
  async pollStatus(submissionId: string): Promise<void> {
    ...
    try {
      ...
      if (response.StatusCode === "00") {
        ...
        submission.responseCufe = this.extractCufeFromResponse(
          response.XmlBytes,
        );
        ...
        if (response.XmlBytes) {
          await this.storage.save(
            submission.tenantId,
            "dian-response",
            `response-${submission.id}.zip`,
            response.XmlBytes,
          );
          submission.responseZipPath = this.storage.getFullPath(
            submission.tenantId,
            "dian-response",
            `response-${submission.id}.zip`,
          );
  ...
  private createZip(contentBase64: string): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      archive.append(Buffer.from(contentBase64, "base64"), {
        name: "content.xml",
      });
      archive.finalize();
    });
