...
export class ValidationsService {
  ...
  validateInvoice(input: InvoiceValidationInput): void {
    ...
    this.validateDocument(input.customerDocumentType, input.customerDocumentNumber);
  ...
  private validateLines(input: InvoiceValidationInput): void {
    if (!input.lines || input.lines.length === 0) {
      throw new BadRequestException('Debe incluir al menos una línea de factura');
    }
    ...
    for (const [index, line] of input.lines.entries()) {
      ...
      const expectedAmount = parseFloat((line.quantity * line.unitPrice).toFixed(2));
      ...
      if (!TAX_CODE_PATTERN.test(line.taxCode)) {
        throw new BadRequestException(`Línea ${lineNum}: código de impuesto inválido (${line.taxCode}). Valores: ${VALID_TAX_CODES.join(', ')}`);
      }

      if (line.taxCode === '01' && !VALID_TAX_PERCENTS.includes(line.taxPercent)) {
        throw new BadRequestException(
      ...
      if (line.taxPercent < 0 || line.taxPercent > 100) {
        throw new BadRequestException(`Línea ${lineNum}: porcentaje de impuesto fuera de rango (0-100)`);
      }
  ...
  private validateTaxTotals(input: InvoiceValidationInput): void {
    if (!input.taxTotals || input.taxTotals.length === 0) {
      throw new BadRequestException('Debe incluir al menos un total de impuesto');
    }

    let calculatedTotalTax = 0;
    for (const [index, tax] of input.taxTotals.entries()) {
      if (!TAX_CODE_PATTERN.test(tax.taxId)) {
        throw new BadRequestException(`Impuesto ${index + 1}: código inválido (${tax.taxId})`);
      }

      if (tax.taxPercent < 0 || tax.taxPercent > 100) {
        throw new BadRequestException(`Impuesto ${index + 1}: porcentaje fuera de rango`);
      }

      if (tax.taxableAmount < 0) {
        throw new BadRequestException(`Impuesto ${index + 1}: base imponible no puede ser negativa`);
      }

      if (tax.taxAmount < 0) {
        throw new BadRequestException(`Impuesto ${index + 1}: valor de impuesto no puede ser negativo`);
      }

      const expectedTax = parseFloat((tax.taxableAmount * (tax.taxPercent / 100)).toFixed(2));
  ...
  private validateTotals(input: InvoiceValidationInput): void {
    const expectedTotal = parseFloat((input.subtotal + input.totalTax).toFixed(2));
  ...
  private validateDocument(documentType: string, documentNumber: string): void {
    ...
    if (documentType === '31') {
      ...
      if (!CC_PATTERN.test(documentNumber)) {
        throw new BadRequestException('Cédula de ciudadanía debe tener entre 5 y 10 dígitos');
      }
  ...
  private validatePrefix(prefix: string): void {
    ...
    if (!/^[A-Za-z0-9]+$/.test(prefix)) {
      throw new BadRequestException('Prefijo solo permite caracteres alfanuméricos');
    }
  }
  ...
  private validateIssueDate(issueDate: string): void {
    ...
    if (date < maxPast) {
      throw new BadRequestException('La fecha de emisión no puede ser mayor a 5 años en el pasado');
    }
  }
}
