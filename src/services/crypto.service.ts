...
export class CryptoService {
  ...
  constructor(private configService: ConfigService) {
    ...
    const historyJson = this.configService.get<string>(
    ...
    if (historyJson) {
      try {
        ...
        this.logger.log(
          `Cargadas ${Object.keys(history).length} claves previas del historial`,
        );
      } catch {
  ...
  decrypt(
    inputOrCiphertext: DecryptInput | string,
    aadOrIv?: string,
    authTagOrAad?: string,
    aad?: string,
  ): string {
    ...
    if (typeof inputOrCiphertext === "string") {
      result = {
        ciphertext: inputOrCiphertext,
        iv: aadOrIv!,
        authTag: authTagOrAad!,
      };
      aadValue = aad;
    } else {
    ...
    if (result.integrityHash) {
      ...
      if (computed !== result.integrityHash) {
        throw new Error(
          "Integrity check failed: el contenido descifrado no coincide con el hash",
        );
      }
    }
  ...
  addKey(version: number, keyHex: string): void {
    ...
    if (key.length !== 32) {
      throw new Error(
        `La clave debe ser de 32 bytes (64 hex), recibió ${key.length * 2} hex`,
      );
    }
  ...
  decryptToObject<T = Record<string, any>>(
