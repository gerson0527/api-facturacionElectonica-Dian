...
export class PdfQrService {
  ...
  async generatePdf(
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      doc.text(`$${subtotal.toFixed(2)}`, 310, doc.y - 12, { width: 80, align: 'right' });
      doc.text(`$${subtotal.toFixed(2)}`, 400, doc.y - 12, { width: 100, align: 'right' });
      doc.moveDown(1);
      ...
      doc.text(`$${subtotal.toFixed(2)}`, 400, totalsY, { width: 100, align: 'right' });
      doc.text('Total Impuestos:', 350, doc.y + 15);
      doc.text(`$${totalTax.toFixed(2)}`, 400, doc.y, { width: 100, align: 'right' });
      doc.font('Helvetica-Bold').fontSize(10);
      doc.text('TOTAL:', 350, doc.y + 20);
      doc.text(`$${totalAmount.toFixed(2)}`, 400, doc.y, { width: 100, align: 'right' });
      doc.moveDown(2);

      // QR Code
      const qrY = doc.y + 20;
      doc.image(qrDataUrl, doc.page.width - 250, qrY, { width: 150, height: 150 });
