...
describe("TenantGuard", () => {
    ...
    return {
      switchToHttp: () => ({
        getRequest: () => ({
          ...
          user: userTenantId
            ? { tenant_id: userTenantId, role: userRole || "tenant_user" }
            : undefined,
        }),
      }),
    } as any;
  ...
  it("debe permitir a super_admin aunque el tenant no coincida", () => {
    expect(
      guard.canActivate(mockContext("tenant-1", "tenant-2", "super_admin")),
    ).toBe(true);
  });

  it("debe denegar si X-Tenant-Id no coincide con JWT tenant_id", () => {
    expect(() =>
      guard.canActivate(mockContext("tenant-1", "tenant-2")),
