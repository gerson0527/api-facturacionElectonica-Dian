...
export default function Products() {
  ...
  const [formData, setFormData] = useState({ code: '', name: '', price: 0, taxRate: 19, stock: 0 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ code: '', name: '', price: 0, taxRate: 19, stock: 0 });
      fetchProducts();
    } catch (e) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: '100%', animation: 'fadeIn 0.3s ease-out' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: '0.25rem' }}>Catálogo de ítems facturables</p>
        </div>
        ...
        <div className="card-header" style={{ padding: '1.5rem 1.5rem 0 1.5rem' }}>
          <div className="input-group" style={{ marginBottom: 0, width: '300px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" className="input-field" placeholder="Buscar producto..." style={{ paddingLeft: '2.5rem' }} />
            </div>
          </div>
        </div>
        ...
        <div className="table-container" style={{ border: 'none', boxShadow: 'none', flex: 1, marginTop: '1rem' }}>
          <table className="table">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '2rem' }}>Cargando...</td>
                </tr>
              ) : products.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>No hay productos registrados.</td>
                </tr>
              ) : (
                products.map((p) => (
                  <tr key={p.id}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div className="card" style={{ width: '400px', animation: 'scaleIn 0.2s ease-out' }}>
            <h2 style={{ marginBottom: '1.5rem' }}>Nuevo Producto</h2>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label className="input-label">Código SKU</label>
                <input className="input-field" required value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" className="input-field" required value={formData.price} onChange={e => setFormData({...formData, price: Number(e.target.value)})} />
              </div>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <label className="input-label">IVA (%)</label>
                  <input type="number" className="input-field" required value={formData.taxRate} onChange={e => setFormData({...formData, taxRate: Number(e.target.value)})} />
                </div>
                <div style={{ flex: 1 }}>
                  <label className="input-label">Stock Inicial</label>
                  <input type="number" className="input-field" required value={formData.stock} onChange={e => setFormData({...formData, stock: Number(e.target.value)})} />
                </div>
              </div>
